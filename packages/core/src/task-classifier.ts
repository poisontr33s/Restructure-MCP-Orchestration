...
export class TaskClassifier {
  private categoryKeywords: Map<TaskCategory, string[]> = new Map([
    [TaskCategory.ARCHITECTURE, [
      'design', 'architecture', 'system', 'structure', 'patterns', 'scalability',
      'microservices', 'monolith', 'distributed', 'components', 'modules'
    ]],
    [TaskCategory.INFRASTRUCTURE, [
      'deploy', 'infrastructure', 'server', 'cloud', 'docker', 'kubernetes', 
      'ci/cd', 'pipeline', 'environment', 'hosting', 'devops'
    ]],
    [TaskCategory.PERFORMANCE, [
      'performance', 'optimize', 'speed', 'slow', 'bottleneck', 'memory',
      'cpu', 'cache', 'latency', 'throughput', 'profiling'
    ]],
    [TaskCategory.CODE_ANALYSIS, [
      'review', 'analyze', 'audit', 'refactor', 'quality', 'maintainability',
      'code smell', 'technical debt', 'complexity', 'metrics'
    ]],
    [TaskCategory.UI_UX, [
      'ui', 'ux', 'interface', 'design', 'user', 'frontend', 'component',
      'responsive', 'accessibility', 'usability', 'wireframe'
    ]],
    [TaskCategory.TESTING, [
      'test', 'testing', 'unit', 'integration', 'e2e', 'coverage', 'qa',
      'quality', 'validation', 'verification', 'mock'
    ]],
    [TaskCategory.SECURITY, [
      'security', 'vulnerability', 'auth', 'authorization', 'encryption',
      'secure', 'audit', 'compliance', 'privacy', 'threat'
    ]],
    [TaskCategory.PROJECT_MANAGEMENT, [
      'manage', 'plan', 'schedule', 'timeline', 'resource', 'allocate',
      'priority', 'milestone', 'track', 'coordinate'
    ]],
    [TaskCategory.RESEARCH, [
      'research', 'investigate', 'explore', 'study', 'analyze', 'understand',
      'learn', 'discover', 'evaluate', 'compare'
    ]],
    [TaskCategory.DOCUMENTATION, [
      'document', 'documentation', 'readme', 'guide', 'tutorial', 'api',
      'spec', 'specification', 'manual', 'wiki'
    ]],
    [TaskCategory.DEBUGGING, [
      'debug', 'bug', 'error', 'issue', 'problem', 'fix', 'troubleshoot',
      'diagnose', 'resolve', 'investigate'
    ]],
    [TaskCategory.CREATIVE_PROBLEM_SOLVING, [
      'creative', 'innovative', 'brainstorm', 'ideate', 'solution', 'approach',
      'alternative', 'novel', 'unique', 'breakthrough'
    ]],
    [TaskCategory.TEAM_COORDINATION, [
      'team', 'collaborate', 'coordinate', 'communicate', 'sync', 'align',
      'meeting', 'standup', 'retrospective', 'planning'
    ]],
    [TaskCategory.MENTORING, [
      'mentor', 'teach', 'guide', 'help', 'support', 'training', 'learning',
      'knowledge', 'skill', 'coaching'
    ]]
  ]);
  ...
  public getRecommendedAgents(classification: TaskClassification): string[] {
    ...
    if (classification.complexityLevel === ComplexityLevel.EXPERT || 
  ...
  private determineCollaborationStyle(
    ...
    if (taskLower.includes('team') || taskLower.includes('collaborate')) {
      return scope === ScopeLevel.STRATEGIC ? CollaborationStyle.CROSS_FUNCTIONAL : CollaborationStyle.SMALL_TEAM;
    }
  ...
  private extractDomainKeywords(taskLower: string): string[] {
    const technicalTerms = [
      ...
      'microservices', 'api', 'database', 'sql', 'nosql', 'redis',
      'machine learning', 'ai', 'blockchain', 'iot', 'mobile'
    ];
  ...
  private identifyTaskPatterns(
    ...
    if (taskLower.includes('iterate') || taskLower.includes('refine') || taskLower.includes('improve')) {
      patterns.push(TaskPattern.ITERATIVE_REFINEMENT);
    }
    
    if (taskLower.includes('parallel') || taskLower.includes('simultaneous') || taskLower.includes('concurrent')) {
      patterns.push(TaskPattern.PARALLEL_EXECUTION);
    }
    
    if (taskLower.includes('step') || taskLower.includes('phase') || taskLower.includes('sequence')) {
      patterns.push(TaskPattern.SEQUENTIAL_PIPELINE);
    }
    
    if (taskLower.includes('explore') || taskLower.includes('discover') || taskLower.includes('research')) {
    ...
    if (taskLower.includes('integrate') || taskLower.includes('combine') || taskLower.includes('merge')) {
  ...
  private calculateConfidence(categoryScores: Map<TaskCategory, number>, taskLower: string): number {
