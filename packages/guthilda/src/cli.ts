...
async function handleStatus(guthilda: CaptainGuthilda, subcommand?: string) {
  ...
  console.log(`   AI Services: ${getStatusIcon(status.systems.aiServices)} ${status.systems.aiServices}`);
  console.log(`   Automation: ${getStatusIcon(status.systems.automation)} ${status.systems.automation}`);
  console.log(`   Monitoring: ${getStatusIcon(status.systems.monitoring)} ${status.systems.monitoring}`);
