...
class JsonToMdSessionOptimizer {
  ...
  async truncateAndOptimize(messages) {
    ...
    console.log(
      `   📊 Optimized: ${intelligence.critical.length}C + ${intelligence.high.length}H + ${intelligence.medium.length}M + ${intelligence.low.length}L`
    );
  ...
  extractTools(message) {
    if (message.type === 'assistant' && message.message?.content) {
      if (Array.isArray(message.message.content)) {
        return message.message.content.filter((c) => c.type === 'tool_use').map((c) => c.name);
      }
    }
    return [];
  }
  ...
  calculateSignificance(content) {
    ...
    const matches = significanceWords.filter((word) => content.toLowerCase().includes(word)).length;
...
**🎯 Session Optimized**: This .md.session contains ${intelligenceScore}% intelligence in maximum-performance format, auto-preloadable with zero redundancy.`;
  ...
  calculateOverallIntelligence(intelligence) {
    const totalItems =
      intelligence.critical.length +
      intelligence.high.length +
      intelligence.medium.length +
      intelligence.low.length;
  ...
  extractSessionVision(intelligence) {
    const visionItems = intelligence.critical
      .concat(intelligence.high)
