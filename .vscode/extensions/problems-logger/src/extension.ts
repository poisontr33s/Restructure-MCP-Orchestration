  ...
  const norm = items
    .map((x) => ({
      ...
      problems: [...x.problems]
        ...
        .sort((a, b) =>
          (a.severity + '' + a.source + '' + a.message).localeCompare(
            b.severity + '' + b.source + '' + b.message
          )
        ),
    }))
...
async function dumpProblems(): Promise<string> {
  ...
  const diagnostics: ReadonlyArray<[vscode.Uri, readonly vscode.Diagnostic[]]> =
    vscode.languages.getDiagnostics();
  const items = diagnostics
    .map(([uri, diags]: [vscode.Uri, readonly vscode.Diagnostic[]]) => ({
      file: uri.fsPath,
  ...
  await fs.promises.writeFile(
    outPath,
    JSON.stringify({ when: new Date().toISOString(), signature, items }, null, 2)
  );
  return outPath;
}
...
async function dumpTerminals(): Promise<string> {
  ...
  const terms = vscode.window.terminals.map((t: vscode.Terminal) => ({
    name: t.name,
    creationOptions: t.creationOptions,
  }));
  await fs.promises.writeFile(
    outPath,
    JSON.stringify({ when: new Date().toISOString(), terminals: terms }, null, 2)
  );
  return outPath;
}
...
async function dumpTasks(): Promise<string> {
  ...
  await fs.promises.writeFile(
    outPath,
    JSON.stringify({ when: new Date().toISOString(), running }, null, 2)
  );
  return outPath;
}
...
async function dumpEditors(): Promise<string> {
  ...
  await fs.promises.writeFile(
    outPath,
    JSON.stringify({ when: new Date().toISOString(), editors }, null, 2)
  );
  return outPath;
}
...
export function activate(context: vscode.ExtensionContext) {
  ...
  function computeCombinedSignature(): string {
    ...
    const sessions = Array.isArray(dbg?.sessions)
      ? dbg.sessions
          .map((s: any) => ({ name: s?.name, type: s?.type }))
          .sort((a: any, b: any) =>
            (String(a.type) + String(a.name)).localeCompare(String(b.type) + String(b.name))
          )
      : [];
    const bps = Array.isArray((vscode as any).debug?.breakpoints)
      ? ((vscode as any).debug.breakpoints as any[]).length
      : vscode.debug.breakpoints.length;
    const scmAny: any = (vscode as any).scm;
    const repos = Array.isArray(scmAny?.repositories)
      ? scmAny.repositories
          .map((r: any) => ({ label: r?.provider?.label, root: r?.rootUri?.fsPath || '' }))
          .sort((a: any, b: any) =>
            (String(a.label) + String(a.root)).localeCompare(String(b.label) + String(b.root))
          )
      : [];
  ...
  const cmd = vscode.commands.registerCommand('problemsLogger.dump', async () => {
    try {
      const [p, t, k, e] = await Promise.all([
        dumpProblems(),
        dumpTerminals(),
        dumpTasks(),
        dumpEditors(),
  ...
  const dumpIfChanged = vscode.commands.registerCommand(
    'problemsLogger.dumpIfChanged',
    async () => {
      try {
        const ws = getWorkspaceFolder();
        if (!ws) {
        ...
        const sig =
          scope === 'combined'
            ? computeCombinedSignature()
            : computeProblemsSignature(getProblemsItems() as any);
        const key = 'problemsLogger.lastSig';
        const last = context.globalState.get<string>(key);
        if (last !== sig) {
          const full =
            vscode.workspace
              .getConfiguration('problemsLogger')
              .get<boolean>('autoDumpFullSnapshot') === true;
          if (full) {
          ...
          await fs.promises.mkdir(outDir, { recursive: true });
          await fs.promises.writeFile(
            path.join(outDir, 'last-problems-sig.json'),
            JSON.stringify({ when: new Date().toISOString(), signature: sig }, null, 2)
          );
