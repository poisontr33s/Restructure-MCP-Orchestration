...
const info = {
  ...
  os: { platform: os.platform(), release: os.release(), arch: os.arch(), cpus: os.cpus().length, totalMemGB: +(os.totalmem()/1e9).toFixed(2) },
  ...
  env: {
    ...
    PATH_has_local_bin: (process.env.Path || process.env.PATH || '').toLowerCase().includes('c:/users/erdno/.local/bin')
  },
};
...
console.log(`OS: ${info.os.platform} ${info.os.release} ${info.os.arch} CPUs=${info.os.cpus} Mem=${info.os.totalMemGB}GB`);
console.log(`Node: ${info.node}`);
for (const [k,v] of Object.entries(info.tools)) console.log(`${k}: ${v || 'missing'}`);
console.log(`Keys -> OPENAI:${info.env.OPENAI_API_KEY} GEMINI:${info.env.GEMINI_API_KEY} ANTHROPIC:${info.env.ANTHROPIC_API_KEY} EXP:${info.env.MCP_EXPERIMENTAL}`);
