...
function runGeminiCapture(prompt: string): Promise<string> {
  return new Promise((resolve) => {
    ...
    const child = child_process.spawn('gemini', args, {
      env: withEnvPath('gemini'),
...
function getIncludeDirs(): string[] {
  ...
  const dirs = cfg.get<string[]>('includeDirectories') || [
    'packages',
    'scripts',
    'docs',
    '.vscode',
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const askSelection = vscode.commands.registerCommand('geminiCli.askSelection', async () => {
    ...
    const instruction = await vscode.window.showInputBox({
      prompt: 'Ask Gemini about this selection/file',
      value: 'Explain, improve, or fix',
    });
  ...
  const askFile = vscode.commands.registerCommand('geminiCli.askFile', async () => {
    ...
    const instruction = await vscode.window.showInputBox({
      prompt: 'Ask Gemini about this file',
      value: 'Summarize and suggest improvements',
    });
    if (instruction === undefined) return;
  ...
  const applyLast = vscode.commands.registerCommand('geminiCli.applyLastCodeblock', async () => {
    const data = context.globalState.get<{ code: string; lang?: string }>(
      'geminiCli.lastCodeblock'
    );
    if (!data) {
      return vscode.window.showWarningMessage('No code block available from last response');
    }
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
      return vscode.window.showWarningMessage('No active editor');
