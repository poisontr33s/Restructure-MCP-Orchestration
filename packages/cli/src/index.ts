...
program.name('mcp').description('MCP Orchestration System CLI').version('1.0.0');
    ...
    process.on('SIGINT', async () => {
      ...
      try {
        ...
        shutdownSpinner.fail(
          chalk.red(`Failed to shutdown: ${error instanceof Error ? error.message : String(error)}`)
        );
        process.exit(1);
      }
    });
...
program
  ...
  .action(async (options) => {
    ...
    try {
      ...
      status.servers.forEach(
        (server: { name: string; type: string; status: string; port: number; pid?: number }) => {
          let statusColor;
          ...
          console.log(
            `  - ${chalk.cyan(server.name)} (${server.type}): ${statusColor(server.status)}`
          );
        }
      );
      ...
      console.log(
        `  - Memory: ${formatBytes(status.system.memory.free)} free of ${formatBytes(status.system.memory.total)}`
      );
      ...
      .action(async (server, options) => {
        ...
        try {
          ...
          const response = await fetch(
            `http://localhost:${parentOptions.port}/api/servers/${server}/start`,
            {
              method: 'POST',
            }
          );
    ...
    new Command('stop')
      ...
      .action(async (server, options) => {
        ...
        try {
          const parentOptions = options.parent.opts();
          const response = await fetch(
            `http://localhost:${parentOptions.port}/api/servers/${server}/stop`,
            {
              method: 'POST',
            }
          );
    ...
    const answers = await inquirer.prompt([
      {
        ...
        validate: (value) =>
          value != null && value > 0 && value < 65536 ? true : 'Please enter a valid port number',
      },
      {
        type: 'checkbox',
        name: 'servers',
