...
export class DuckDuckGoServer extends BaseServer {
   ...
   * @param res HTTP response
   */
  private async handleSearchRequest(searchParams: URLSearchParams, res: ServerResponse): Promise<void> {
    const query = searchParams.get('q');
    const limit = searchParams.get('limit') ? parseInt(searchParams.get('limit') as string, 10) : this.MAX_RESULTS;
  ...
  private async callDuckDuckGoApi(query: string, _limit: number, _safeSearch: string): Promise<any> {
    try {
      ...
      this.log(`DuckDuckGo API error: ${error instanceof Error ? error.message : String(error)}`, 'ERROR');
      throw error;
    }
  }
}
